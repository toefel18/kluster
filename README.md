...
FOR EACH (request) {
    ...
    WHILE (processed) {
        TRY {
            ...
            produceToErrorTopic(createErrorResponse(request, exception))
            processed = true
        }
    }
}
...
Topic:
- ALL: None.
- ONE: Requests are ditrubuted over all nodes. 

### Consistency

The client can wait for the first (or all when it was a request to ALL) response on RESPONSE topic to see if the result
